...
def create_bug_ticket(jira_url, jira_email, jira_api_token, project_key, summary, description):
    ...
    # Step 2: Link bug to Test Execution and Test Case
    link_url = f"{jira_url}/rest/api/3/issueLink"

    def link_issues(inward, outward):
        link_payload = {
            "type": {"name": "Relates"},  # You can change link type (e.g., "Blocks", "Cloners")
            "inwardIssue": {"key": inward},
            "outwardIssue": {"key": outward}
        }
        link_resp = requests.post(link_url, auth=auth, headers=headers, json=link_payload)
        if link_resp.status_code == 201:
            print(f"🔗 Linked {inward} ↔ {outward}")
        else:
            print(f"⚠️ Failed to link {inward} ↔ {outward}: {link_resp.status_code} - {link_resp.text}")

    # Link bug → Test Execution
    link_issues(bug_key, TEST_EXEC_KEY)
    # Link bug → Test Case
    link_issues(bug_key, TEST_CASE_KEY)
...
def update_xray_test_status(
    ...
    if test_status == "FAILED" and jira_url and jira_email and jira_api_token and jira_project_key:
        ...
        summary = f"{version_name} creation failed"
        description = f"version already exists"
